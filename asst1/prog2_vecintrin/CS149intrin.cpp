...
// NOTE: `first` will init first N to be one 
//
// `first` default to 4
//
// `first: 2` -> get this `1100`
...
// NOTE: count how many `1` in the mask
...
// NOTE: set the value to the vector register if the vector lane is active
//
// when the `mask` is `1`, the `vecResult` will be set to `value` or else keep
template <typename T>
...
// NOTE: add up pair to pair of elements
//
// 1100 -> 2200
//
// 2131 -> 3344
...
// NOTE: 
// if `vec` is 1010 then `vecResult` is `1100`
//              ||                        ||
// change the inner two value order
template <typename T>
